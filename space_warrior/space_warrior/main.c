...
void draw_object(SDL_Renderer *renderer, SDL_Texture *textures, spc_obj_t *obj)
{
    ...
    if(obj->type==spc_obj_star) { // Star
        ...
        if(obj->type==spc_obj_asteroid)
            SDL_RenderCopyEx(renderer, textures, &src, &dest, obj->z*(obj->active%32-16)/120, NULL, 0);
        else
...
int key_control(spc_obj_t *obj, float *vx, float *vy)
{
    ...
    if(event.type==SDL_KEYDOWN) {
        switch(event.key.keysym.sym){
            case SDLK_LEFT: *vx=20; break;
            case SDLK_RIGHT: *vx=-20; break;
            case SDLK_UP: *vy=20; break;
            case SDLK_DOWN: *vy=-20; break;
...
int main(int argc, char *argv[])
{
    ...
    tempSurface=SDL_LoadBMP("/Users/Leon/Documents/Leon E/DHBW Mannheim/Programmieren 1/C/Übungen/space_warrior/space_warrior/901-space_warrior.bmp"); // load textures
